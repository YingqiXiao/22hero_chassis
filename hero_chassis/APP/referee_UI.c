...
void referee_usart_task()
{
	...
	float    Capacitance_X;
		...
		if(UI_PushUp_Counter % 91 == 0) //动态UI预绘制 图形
		{
		UI_Draw_Float (&UI_Graph2.Graphic[0], "201", UI_Graph_Add, 3, UI_Color_Orange, 22, 3, 3, 1355, 632, 1.0f);   //Pith轴角度
		...
		{
			UI_Draw_String(&UI_String1.String, "305", UI_Graph_Add, 2, UI_Color_White, 18, 6, 3,  38, 650, "follow");//超级电容
		...
		{
			UI_Draw_String(&UI_String2.String, "307", UI_Graph_Add, 2, UI_Color_White,18, 4, 3,  38, 750, "fric");//小陀螺	
			UI_PushUp_String(&UI_String2, Robot_ID_Current);	
		}
		...
		if(UI_PushUp_Counter % 21 == 0) //动态UI更新 圆圈位置和颜色
		{
			...
			UI_Draw_Float (&UI_Graph2.Graphic[0], "201", UI_Graph_Change, 3, UI_Color_Orange, 22, 3, 3, 1355, 632, gimbal_move.imu->imu_pitch / 100.0f );   //Pith轴角度
			UI_PushUp_Graphs(2, &UI_Graph2, Robot_ID_Current);
...
//				}
//				if( supercap_reboot_flag==0)//重启标志位
//				{
