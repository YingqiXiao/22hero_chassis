...
{
	float temp,temp2;
if(gimbal_move.motor_yaw.actual_angle<3.90f)
		temp=gimbal_move.motor_yaw.actual_angle+360.0f;
	else temp=gimbal_move.motor_yaw.actual_angle;
	temp2=temp-3.90f;	
...
{		
	chassis_move.wz_set = 6.0f;
	float b=chassis_move.vx_set;
	float a=chassis_move.vy_set;
	chassis_move.vy_spin_set = (float)(b*sin(chassis_move.chassis_gimbal_angle) + a*cos(chassis_move.chassis_gimbal_angle));
	chassis_move.vx_spin_set = (float)(b*cos(chassis_move.chassis_gimbal_angle) - a*sin(chassis_move.chassis_gimbal_angle));
}
...
void chassis_wheel_spin_speed(const float vx_set, const float vy_set, const float wz_set, float wheel_speed[4])
{
    wheel_speed[1] = (-vx_set - vy_set - MOTOR_DISTANCE_TO_CENTER * wz_set);
    wheel_speed[3] = (vx_set - vy_set - MOTOR_DISTANCE_TO_CENTER * wz_set);
    wheel_speed[2] = (vx_set + vy_set - MOTOR_DISTANCE_TO_CENTER * wz_set);
...
void chassis_control_loop(chassis_move_t *chassis_move_control_loop)
{
    ...
    uint8_t i = 0;
	get_chassis_gimbal_angle();
	
	if(chassis_move_control_loop->channel_s->sL == 3)
	{
	chassis_follow();
	...
	if(chassis_move_control_loop->channel_s->sL == 2)
	{
	chassis_spin();
	chassis_wheel_spin_speed(chassis_move_control_loop->vx_spin_set,
                                          chassis_move_control_loop->vy_spin_set, 
									      chassis_move_control_loop->wz_set, wheel_speed);
	}	

	if(chassis_move_control_loop->channel_s->sL == 1)
	{
	chassis_wheel_spin_speed(0.0f,0.0f,0.0f, wheel_speed);
	}		
...
void chassis_init(chassis_move_t *chassis_move_init)
{
		...
		const static float chassis_buffer_pid[3] = 
		    {1.0f,0,0};		
		...
		chassis_move_init->channel_s = get_channel_measure_point();
		...
		PID_init(&chassis_move_init->buffer_pid, 
							chassis_buffer_pid, 
								10, 
									0,
										0);		
...
void chassis_follow(void)
{
	...
	PID_calc(chassis_move.chassis_follow_pid,gimbal_move.motor_yaw.actual_angle,3.90f);
	chassis_move.wz_set = - chassis_move.chassis_follow_pid->out;
}
